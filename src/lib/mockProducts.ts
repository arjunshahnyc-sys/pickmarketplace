// Mock product catalog used as a fallback when live search returns too few
// results. Moved out of the /api/search route so ask/similar/search can share
// one in-process implementation instead of fetching each other over HTTP.
...
export function populateUrls(products: ProductResult[], query: string): ProductResult[] {
...
export function searchMockProducts(query: string): ProductResult[] {
